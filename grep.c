...
void filename(char*);  void gdelete(void);  char *getblock(unsigned int atl, int iof); int getchr(void);
...
void search_file(char* filename, const char* searchfor);
...
void readfile(char* passed_file){
	setnoaddr(); 
...
void filename(char* passed_file) { 
	strcpy(file, passed_file);
	strcpy(savedfile, passed_file);
}
